...
function sdiff(ours) {
  ...
  if (
    this.data.has(ours) &&
    !(this.data.get(ours) instanceof _es6Set.default)
  ) {
    throw new Error('Key '.concat(ours, ' does not contain a set'));
  }

  for (
    var _len = arguments.length,
      theirs = new Array(_len > 1 ? _len - 1 : 0),
      _key = 1;
    _key < _len;
    _key++
  ) {
  ...
  theirs.forEach(function(key) {
    if (
      _this.data.has(key) &&
      !(_this.data.get(key) instanceof _es6Set.default)
    ) {
  ...
  var difference = new _es6Set.default(
    (0, _arrayFrom.default)(ourSet).filter(function(ourValue) {
      return theirSets.reduce(
        function(isUnique, set) {
