...
function zinterstore(destKey, numKeys) {
  ...
  if (
    parseInt(numKeys, 10) !== (arguments.length <= 2 ? 0 : arguments.length - 2)
  ) {
  ...
  for (
    ...
    if (
      ...
      this.data.get(
        arguments.length <= i + 2 ? undefined : arguments[i + 2]
      ) instanceof _es6Map2.default
    ) {
      srcMaps.push(
        this.data.get(arguments.length <= i + 2 ? undefined : arguments[i + 2])
      );
    }
  }
  ...
  var intersected = _lodash.intersectionBy.apply(
    undefined,
    _toConsumableArray(inputs).concat(['value'])
  );
